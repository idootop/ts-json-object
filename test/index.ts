  ...
  async run() {
    ...
    for (let i = 0; i < this._tests.length; ++i) {
      ...
      process.stdout.write(
        `[${("" + (i + 1)).padStart(
          COUNT.length
        )}/${COUNT} ${RUN}${test.name.padEnd(name_len)}${CLEAR}] - ${BOLD}`
      );
      try {
        let ret = test.run();
...
let test = new Test([
  ...
  },
  {
    name: "SafeJSON",
    run: () => {
      class Item extends SafeJSON<Item> {
        @optional
        name?: string;
      ...
      // 构造参数可空
      let test = new Item({ name: "test" });
      // 对象字段不可空
      assert.ok(test.avatar === "");
