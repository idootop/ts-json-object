...
export class JSONObject extends Object {
  private static newValue(
    ...
    if (typeof value == "object") {
      if (Array.isArray(value)) {
        let array_meta = Reflect.hasMetadata(__array, target, key)
          ? Reflect.getMetadata(__array, target, key)
          : null;
        if (array_meta && value.length) {
          ...
          for (let i = 0; i < value.length; ++i) {
            ...
            if (typeof e == "object") {
              if (Array.isArray(e)) {
                // (at least) 2 dim array, throw an unsupported error
                throw new JSONTypeError(
                  `${target.constructor.name}.${key}: array type cannot be array, consider using @passthrough and @validate`
                );
              }
            ...
            if (expected_type != value_type) {
              throw new JSONTypeError(
                `${target.constructor.name}.${key} array element requires type '${expected_type}', got '${value_type}' instead`
              );
            }
            ret.push(new_e);
          }
  ...
  private static set(target: any, key: string, value: any) {
    let new_value: any;
    let design_type = Reflect.getMetadata("design:type", target, key);
    if (design_type === undefined) {
      throw new Error("Meta data not defined");
    }
    if (Reflect.hasMetadata(__union, target, key)) {
      let values = <Array<any>>Reflect.getMetadata(__union, target, key);
      let is_valid = false;
      ...
      if (!is_valid) {
        throw new JSONTypeError(
          `${target.constructor.name}.${key} requires one of the following values:[${values}], got '${value}' instead`
        );
      }
    }
    if (
      Reflect.hasMetadata(__passthrough, target, key) &&
      Reflect.getMetadata(__passthrough, target, key) == true
    ) {
      new_value = value;
    } else {
      // Run custom assignment if exists
      if (Reflect.hasMetadata(__custom, target, key)) {
        let code = <Validator>Reflect.getMetadata(__custom, target, key);
        let ret = code(target, key, value);
        if (ret !== undefined) new_value = ret;
      } else {
        new_value = JSONObject.newValue(target, value, design_type, key);
        // If not date
        if (design_type.name == "Date") {
          if (
            typeof new_value !== "object" ||
            typeof new_value.getTime !== "function"
          ) {
            throw new JSONTypeError(
              `${
                target.constructor.name
              }.${key} requires a type that can be converted to a date, got '${typeof value}' instead`
            );
          }
          if (isNaN(new_value.getTime())) {
            throw new JSONTypeError(
              `${
                target.constructor.name
              }.${key} requires a type that can be converted to a date, got '${typeof value}(${value})' instead`
            );
          }
        } else {
          ...
          if (expected_type != value_type) {
            throw new JSONTypeError(
              `${target.constructor.name}.${key} requires type '${expected_type}', got '${value_type}' instead`
            );
          }
        }
        ...
        for (let symbol of symbols) {
          if (Reflect.hasMetadata(symbol, target, key)) {
            let code = <Validator>Reflect.getMetadata(symbol, target, key);
            code(target, key, new_value);
          }
        }
        // Run integer validator
        if (Reflect.hasMetadata(__integer, target, key)) {
          JSONObject.validateNumeric(target, key, new_value);
          if (new_value != Math.floor(new_value)) {
            throw new JSONTypeError(
              `${target.constructor.name}.${key} should be an integer value, got ${new_value} instead`
            );
          }
        }
      }
    }
    (<any>target)[key] = new_value;
  }

  private static get(target: any, key: string): any {
  ...
  constructor(json?: any) {
    ...
    for (let key of json_keys) {
      ...
      if (json_value === undefined || json_value === null) {
        if (
          JSONObject.get(this, key) === undefined &&
          !JSONObject.isOptional(this, key)
        ) {
          throw new JSONTypeError(
            `${this.constructor.name}.${key} is required`
          );
        }
        (<any>this)[key] = json_value; // undefined or null, exactly as specified (or not specified :-) )
      } else {
        JSONObject.set(this, key, json_value);
      }
    }
  }
  ...
  private static validateNumeric(target: any, key: string, value: any) {
    if (typeof value !== "number") {
      throw new JSONTypeError(
        `${target.constructor.name}.${key}: requires numeric type for validation operators`
      );
    }
  }
  ...
  public static gt(value: number) {
    ...
    let validator = function (
      ...
      JSONObject.validateNumeric(object, key, jsonValue);
      if (!(jsonValue > capture)) {
        throw new JSONTypeError(
          `${object.constructor.name}.${key}: ${jsonValue} > ${capture} requirement failed`
        );
      }
    };
...
export class StrictJSON<T extends any> extends JSONObject {
  constructor(json?: T) {
    super(json ?? {});
  }
}
